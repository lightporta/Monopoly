# 《仙境海岸·大富翁》产品需求文档（PRD V3）

> **文档版本**：V3.0
> **生成日期**：2026-07-13
> **适用范围**：含四大海洋板块 + 联机版 + UI 优化的完整版本
> **前序文档**：V1.0 Demo 版（`.trae/documents/PRD.md`）、V2.0 联机版改造
...
| 胜利方式 | 触发条件 | 优先级 |
|---|---|---|
| **破产胜利** | 仅剩 1 名玩家未破产 | **高**（同动作触发两者时以破产为准） |
| **仙境铁三角胜利** | 同时持有烟台山 + 蓬莱阁 + 养马岛 三处所有权，**且每处地标至少建有 1 级房屋，且总资产 > 30000 元** | 低 |

### 8.2 关键判定

- **铁三角三重门槛**：①拥有三地标所有权 ②各地标均建有房屋（≥1级）③总资产 > 30000，三者全满足才触发
- **铁三角抵押不影响所有权判定**：只要所有权属于该玩家即算集齐
- **破产优先于铁三角**：同动作触发两者时以破产胜利为准
- **两种条件实时检测**：每次状态变更后立即判定
...
```
总资产 = 现金 + 地产估值 + 建筑估值 + 养殖场估值 + 装备估值 + 投资成本
```
...
> **文档结束** | V3.0 PRD | 2026-07-13 | 配套文档：[技术架构 TD V3](./TechnicalArchitecture-v3.md)、[游戏设计 V3](./game-design-v3.md)、[部署指南](./deployment-guide.md)
